...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    // Blue color scheme
    ImU32 col = GetColorU32((held && hovered) ? ImVec4(0.0f, 0.5f, 1.0f, 1.0f) : hovered ? ImVec4(0.15f, 0.15f, 0.15f, 1.0f) : ImVec4(0.1f, 0.1f, 0.1f, 1.0f));

    // Render
    RenderNavHighlight(bb, id);
    RenderFrame(bb.Min, bb.Max, col, true, 4.0f); // Slight rounding
...
// Structure to hold the animation state of the checkbox
struct CheckboxState
{
    ImVec4 checkmark = ImVec4(0, 0, 0, 0);
    ImVec4 framecol = ImVec4(25, 25, 25, 255);
};
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    if (it_anim == anim.end())
    {
        anim.insert({ id, CheckboxState() });
        it_anim = anim.find(id);
    }

    // Blue color scheme
    ImVec4 active_bg_color = ImVec4(1.0f, 0.84f, 0.0f, 1.0f);
    ImVec4 inactive_bg_color = ImVec4(0.1f, 0.1f, 0.1f, 1.0f);
    ImVec4 hover_bg_color = ImVec4(0.15f, 0.15f, 0.15f, 1.0f);

    ImVec4 active_check_color = ImVec4(1.0f, 1.0f, 1.0f, 1.0f);
    ImVec4 inactive_check_color = ImVec4(0, 0, 0, 0);
    ImVec4 hover_check_color = ImVec4(0.7f, 0.7f, 0.7f, 1.0f);

    it_anim->second.checkmark = LerpColor(it_anim->second.checkmark, *v ? active_check_color : hovered ? hover_check_color : inactive_check_color, g.IO.DeltaTime * 4.f);
    it_anim->second.framecol = LerpColor(it_anim->second.framecol, *v ? active_bg_color : hovered ? hover_bg_color : inactive_bg_color, g.IO.DeltaTime * 4.f);

    RenderFrame(frame_bb.Min, frame_bb.Max, GetColorU32(it_anim->second.framecol), 0, ROUNDING);
    if (*v || it_anim->second.checkmark.w > 0.0f) // Only render checkmark if it's visible
    {
    ...
    // Adjust label position to be vertically centered with the checkbox
    ImVec2 label_pos = ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x + 3, frame_bb.Min.y + (square_sz - label_size.y) / 2);
    if (label_size.x > 0.f)
        RenderText(label_pos, label);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const ImRect total_bb(bb.Min, bb.Max + ImVec2((arrow_size > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f), 0.0f));
    ItemSize(total_bb, style.FramePadding.y);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    draw->AddText(ImVec2(total_bb.Min.x, frame_bb.GetCenter().y - label_size.y / 2), GetColorU32(ImGuiCol_Text), label);

    draw->AddRectFilled(frame_bb.Min, frame_bb.Max, GetColorU32(ImGuiCol_FrameBgActive), style.FrameRounding);
    draw->AddRectFilled(frame_bb.Min, ImVec2(frame_bb.Min.x + value->second, frame_bb.Max.y), GetColorU32(ImGuiCol_SliderGrabActive), style.FrameRounding);
    draw->AddCircleFilled(ImVec2(frame_bb.Min.x + value->second - 2, frame_bb.GetCenter().y), 6, GetColorU32(ImGuiCol_SliderGrabActive), 30);

    // Render the number without background
...
{
    // Directly call SliderScalar with ImGuiDataType_Float and the provided min/max values
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        if (ColorButton("##ColorButton", col_v4, flags))
        {
            if (!(flags & ImGuiColorEditFlags_NoPicker))
            {
                // Store current color and open a picker
                g.ColorPickerRef = col_v4;
                OpenPopup("color_palette_picker");
                SetNextWindowPos(g.LastItemData.Rect.GetBL() + ImVec2(0.0f, style.ItemSpacing.y));
            }
        }
        if (!(flags & ImGuiColorEditFlags_NoOptions))
        ...
        if (BeginPopup("color_palette_picker"))
        {
            if (g.CurrentWindow->BeginCount == 1)
            {
                picker_active_window = g.CurrentWindow;

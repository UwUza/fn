...
public:
	...
	static std::string ToString(int32_t index)
	{
		...
		uint64_t NamePoolChunk = read<uint64_t>(virtualaddy + (0x16A27140 + 8 * (DecryptedIndex >> 16) + 16)) + 2 * (uint16_t)DecryptedIndex;
		uint16_t Pool = read<uint16_t>(NamePoolChunk);

		if (((Pool ^ 0x25C0) & 0x7FE0) <= 0)
		{
			DecryptedIndex = DecryptIndex(read<int32_t>(NamePoolChunk + 6));
			NamePoolChunk = read<uint64_t>(virtualaddy + (0x16A27140 + 8 * (DecryptedIndex >> 16) + 16)) + 2 * (uint16_t)DecryptedIndex;
		...
		mem::read_physical(PVOID(NamePoolChunk + 2), NameBuffer, Length);
...
{
	if (screen_location.x > 0 && screen_location.x < globals::width && screen_location.y > 0 && screen_location.y < globals::height)
	{
		return true;
	}
	else
